...
- 2026-05-12 — **Round 28 — `LearningScheduler` DQN/PPO dispatch via `AriaConfig.rlAlgorithm` (GAP_AUDIT §220):**
  1. **`AriaConfig.rlAlgorithm`** — New `String = "reinforce"` field in `AriaConfig`; `ConfigStore` key `KEY_RL_ALGORITHM` wired through `save()`/`fromPrefs()`/`migrateFromSharedPrefs()`.
  2. **`LearningScheduler` dispatch** — `runTrainingCycle()` reads `ConfigStore.getBlocking()` and dispatches: `"dqn"` → `DqnNetwork.load/storeTransition×N/trainStep×4/save`; `"ppo"` → `PpoNetwork.load/beginRollout/storeStep×N/endRollout/save`; else → `PolicyNetwork` REINFORCE (default). `AgentEventBus` events `dqn_trained` / `ppo_trained` emitted.
  3. **Public constants** — `DqnNetwork.STATE_DIM` + `PpoNetwork.STATE_DIM/GOAL_DIM` exposed for cross-module use.
  4. **SettingsScreen RL algorithm picker** — REINFORCE/DQN/PPO `FilterChip` row with contextual description added to RL card.
  5. **`scripts/push.sh` lock guard** — `rm -f .git/index.lock` added before `git add -A` to clear stale locks from interrupted pushes.
  6. **`VideoLearner` two-stage dedup fix** — `prevHash` now drives hash-equality check (identical frames) before motion-level check; removes unused `java.io.File` import.
  **Files changed:** `LearningScheduler.kt`, `ConfigStore.kt`, `DqnNetwork.kt`, `PpoNetwork.kt`, `SettingsScreen.kt`, `VideoLearner.kt`, `scripts/push.sh`, `GAP_AUDIT.md`, `replit.md`.
- 2026-05-12 — **Round 27 — Ported donor RL algorithms + scheduler; added unit tests (GAP_AUDIT §214–§220):**
  1. **`DqnNetwork` (§214)** — Real DQN (Mnih 2015): 3-layer MLP (256→256→128→7), circular replay buffer (10 000), frozen target network (hard update every 500 steps), ε-greedy decay (1.0→0.05 over 50 000 steps), Bellman MSE loss, Adam optimizer. `storeTransition()` / `trainStep()` / `selectAction()` API. Saves `rl/dqn_online.bin` + `rl/dqn_target.bin`. Donor `DQNAlgorithm.java` was a pure stub — this is a ground-up real implementation.
  2. **`PpoNetwork` (§215)** — Real PPO-Clip (Schulman 2017): actor MLP (256→256→128→7 softmax) + critic MLP (256→128→1), GAE (γ=0.99 λ=0.95), clipped surrogate objective (ε=0.2), value loss coeff 0.5, entropy bonus 0.01, gradient clipping (norm 0.5), K=3 epochs per rollout. `beginRollout()` / `storeStep()` / `endRollout(lastValue)` API. Saves `rl/ppo_actor.bin` + `rl/ppo_critic.bin`. Donor `PPOAlgorithm.java` was a pure stub — this is a ground-up real implementation.
  3. **`VideoLearner` (§216)** — Port of `donors/learning-video-java/VideoProcessor.java`: coroutines (Dispatchers.IO) replace HandlerThread, `StateFlow<ProcessingState>` replaces callbacks, `MediaMetadataRetriever` extracts 1 fps frames (max 300), perceptual hash (8×8 avg-hash, 64-bit), motion detection (mean-abs-pixel-delta, threshold 3%), action frames stored as `ExperienceTuple` in `ExperienceStore`. 64×64 downscale + explicit bitmap `recycle()` for 4 GB safety.
  4. **`ScheduledTaskManager` (§217)** — Port of `donors/scheduler-java/TaskSchedulerManager.java`: 1 s coroutine ticker replaces `ScheduledExecutorService`, `org.json` JSON persistence replaces Java serialization, `TaskExecutor` suspend interface replaces `ActionExecutor`. Priority sort (HIGH > NORMAL > LOW), max 5 concurrent, max 100 tasks, recurrence (ONCE/INTERVAL_MINS/DAILY/WEEKLY/MONTHLY), `allTasksFlow: StateFlow`, `AgentEventBus` events, RUNNING tasks restored as FAILED on cold-start.
  5. **`DqnNetworkTest` (§218)** — 8 JVM unit tests: ε schedule, replay wrap at capacity, loss updates, valid action index range.
  6. **`PpoNetworkTest` (§219)** — 9 JVM unit tests: action sampling, logProb sign, rollout lifecycle, entropy positivity, critic convergence trend, `totalUpdates` counter.
  **Files changed:** `DqnNetwork.kt` (new), `PpoNetwork.kt` (new), `VideoLearner.kt` (new), `ScheduledTaskManager.kt` (new), `DqnNetworkTest.kt` (new), `PpoNetworkTest.kt` (new), `GAP_AUDIT.md`, `replit.md`.
- 2026-05-12 — **Round 27 hotfix — `PpoNetwork` gradient correctness (GAP_AUDIT §215 amendment):**
  Three critical bugs in `trainOneEpoch()` that would have made the PPO actor learn nothing meaningful:
  1. **Missing `adv` in PPO gradient** — `ppoGrad` was set to `ratio` alone; correct is `adv * ratio * (π_i − δ_{a,i})`. Without the advantage factor, all gradient steps were identically scaled regardless of how good or bad the action was.
  2. **Clipped path used wrong value instead of zero** — When the PPO clip was active, the code still propagated `clipped * (π_i − δ_{a,i})`. PPO-Clip's entire purpose is to zero the gradient when the ratio is out of the trust region; passing a non-zero value defeats it.
  3. **Entropy gradient wrong formula** — Used `c_e · π_i · (−1 − log π_i)` which is ∂H/∂π (not ∂H/∂logit). Correct formula after chain rule through softmax is `c_e · π_i · (log π_i + H)`.
  4. **`sampleCategorical` hot-path allocation** — Was creating a new `java.util.Random()` on every call to `selectAction`; replaced with a class-level `private val rng`.
  **Files changed:** `PpoNetwork.kt`.
- 2026-05-12 — **Round 26 — OpenCL/Vulkan → CPU automatic fallback (GAP_AUDIT §213):**
  - **C++ layer** (`llama_jni.cpp`): After `llama_model_load_from_file` returns `nullptr` with a GPU device in `selected_devices`, immediately retries with `mparams.devices=nullptr` + `n_gpu_layers=0` before crossing the JNI boundary. Covers: missing `libOpenCL.so`, no OpenCL platform, OOM on first GPU alloc. Logs `LOGE` on the original failure and `LOGI` on successful CPU recovery.
  - **Kotlin layer** (`LlamaEngine.kt`): Belt-and-suspenders retry in both `load()` and `loadVision()` — if `nativeLoadModel` still returns `0L` for a non-CPU backend, retries with `gpuBackend="cpu"`, `nGpuLayers=0`, and emits `"gpu_backend_fallback"` event to `AgentEventBus` (keyed `requested`/`fallback`/`model`) so downstream UI can surface a warning chip.
  - **Previously**: model left unloaded while `jniAvailable=true` → `infer()` threw `IllegalStateException` on every agent step → agent appeared frozen / in "stub-like" state with no explanation.
  **Files changed:** `llama_jni.cpp`, `LlamaEngine.kt`, `GAP_AUDIT.md`, `replit.md`.
