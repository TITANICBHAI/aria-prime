...
| 3 | No central orchestrator — `AgentEventBus` is just a topic bus | No component lifecycle, no health checks, no circuit breaking | **`donors/orchestration-java/*`** | Port `CentralAIOrchestrator` + `ComponentRegistry` + `EventRouter` + `CircuitBreaker` + `HealthMonitor` + `DiffEngine` to Kotlin under `core/orchestration/`. Wire `AgentLoop`, `LlamaEngine`, `ObjectDetectorEngine`, `PolicyNetwork`, `LoraTrainer` as registered Components. |
